...
def run_model_pipeline(
    ...
    use_holdout = bool(holdout_periods and holdout_periods > 0)
    holdout_periods = int(holdout_periods) if holdout_periods else 0
    holdout_col_name = holdout_column if holdout_column else None
    ...
    for gvals, gdf in group_list_regular:
        ...
        if use_holdout:
            if holdout_col_name and holdout_col_name in gdf.columns:
                ordered_df = gdf.sort_values(holdout_col_name)
            else:
                if holdout_col_name and holdout_col_name not in gdf.columns:
                    st.warning(
                        f"ℹ️ {group_display_name}: Holdout column '{holdout_col_name}' not found. Using original order."
                    )
                ordered_df = gdf.sort_index()

            if len(ordered_df) <= holdout_periods:
                st.warning(
                    f"⚠️ Skipping holdout for {group_display_name}: only {len(ordered_df)} samples,"
                    f" but holdout requires {holdout_periods}."
                )
                train_df = ordered_df.copy()
                holdout_df = pd.DataFrame()
            else:
                holdout_df = ordered_df.tail(holdout_periods).copy()
                train_df = ordered_df.iloc[:-holdout_periods].copy()
        else:
            train_df = gdf.copy()
            holdout_df = pd.DataFrame()

        present_cols = [c for c in X_columns if c in train_df.columns]
        ...
        for mname, mdl in regular_models.items():
            ...
            for fold_id, (tr_idx, te_idx) in enumerate(kf.split(X_full, y_full), 1):
                ...
                if isinstance(model_copy, (CustomConstrainedRidge, ConstrainedLinearRegression)):
                    ...
                    if '_' in grp_col and grp_col not in train_df.columns:
                        component_keys = grp_col.split('_')
                        if all(k in train_df.columns for k in component_keys):
                            groups_tr_values = train_df.loc[tr_orig_idx, component_keys].astype(str).apply(
                                lambda row: "_".join(row), axis=1
                            )
                            groups_te_values = train_df.loc[te_orig_idx, component_keys].astype(str).apply(
                                lambda row: "_".join(row), axis=1
                            )
                        else:
                            groups_tr_values = train_df.loc[tr_orig_idx, grouping_keys[0]]
                            groups_te_values = train_df.loc[te_orig_idx, grouping_keys[0]]
                    else:
                        if grp_col in train_df.columns:
                            groups_tr_values = train_df.loc[tr_orig_idx, grp_col]
                            groups_te_values = train_df.loc[te_orig_idx, grp_col]
                        else:
                            groups_tr_values = train_df.loc[tr_orig_idx, grouping_keys[0]]
                            groups_te_values = train_df.loc[te_orig_idx, grouping_keys[0]]
                ...
                # Predictions
                pr = train_df.loc[X_te.index].copy()
            ...
            if fold_results:
                ...
                need_final_fit = (use_holdout and not holdout_df.empty) or (
                    isinstance(mdl, RecursiveLeastSquaresRegressor) and getattr(mdl, 'store_history', False)
                )

                if need_final_fit:
                    X_train_final = train_df[present_cols].fillna(0).copy()
                    y_train_final = train_df[target_col].copy()
                    X_holdout = holdout_df[present_cols].fillna(0).copy() if not holdout_df.empty else None
                    y_holdout = holdout_df[target_col].copy() if not holdout_df.empty else None

                    y_train_fit = y_train_final.copy()

                    if log_transform_y:
                        y_train_fit = np.log1p(y_train_fit)

                    cols_to_scale_final = [c for c in std_cols if c in X_train_final.columns] if std_cols else []
                    ...
                    if cols_to_scale_final:
                        ...
                        final_scaler = {c: (m, s) for c, m, s in zip(cols_to_scale_final, sc_final.mean_, sc_final.scale_)}

                    final_model = clone(mdl)

                    if isinstance(final_model, (CustomConstrainedRidge, ConstrainedLinearRegression)):
                        final_model.fit(X_train_final.values, y_train_fit.values, X_train_final.columns.tolist())
                    elif isinstance(final_model, StatsMixedEffectsModel):
                        train_idx = X_train_final.index
                        grp_col = final_model.group_col

                        if '_' in grp_col and grp_col not in train_df.columns:
                            component_keys = grp_col.split('_')
                            if all(k in train_df.columns for k in component_keys):
                                groups_train_values = train_df.loc[train_idx, component_keys].astype(str).apply(
                                    lambda row: "_".join(row), axis=1
                                )
                            else:
                                groups_train_values = train_df.loc[train_idx, grouping_keys[0]]
                        else:
                            if grp_col in train_df.columns:
                                groups_train_values = train_df.loc[train_idx, grp_col]
                            else:
                                groups_train_values = train_df.loc[train_idx, grouping_keys[0]]

                        final_model.fit(X_train_final, y_train_fit, groups_train_values)
                    else:
                        final_model.fit(X_train_final, y_train_fit)

                    if use_holdout and not holdout_df.empty:
                        if isinstance(final_model, (CustomConstrainedRidge, ConstrainedLinearRegression)):
                            holdout_pred = final_model.predict(X_holdout.values)
                        elif isinstance(final_model, StatsMixedEffectsModel):
                            holdout_idx = X_holdout.index
                            grp_col = final_model.group_col

                            if '_' in grp_col and grp_col not in holdout_df.columns:
                                component_keys = grp_col.split('_')
                                if all(k in holdout_df.columns for k in component_keys):
                                    groups_holdout_values = holdout_df.loc[holdout_idx, component_keys].astype(str).apply(
                                        lambda row: "_".join(row), axis=1
                                    )
                                else:
                                    groups_holdout_values = holdout_df.loc[holdout_idx, grouping_keys[0]]
                            else:
                                if grp_col in holdout_df.columns:
                                    groups_holdout_values = holdout_df.loc[holdout_idx, grp_col]
                                else:
                                    groups_holdout_values = holdout_df.loc[holdout_idx, grouping_keys[0]]

                            holdout_pred = final_model.predict(X_holdout, groups_holdout_values)
                        else:
                            holdout_pred = final_model.predict(X_holdout)

                        if log_transform_y:
                            holdout_pred = np.expm1(holdout_pred)
                            holdout_pred = np.maximum(holdout_pred, 0)

                        y_holdout_original = holdout_df[target_col].copy()

                        holdout_r2 = r2_score(y_holdout_original, holdout_pred)
                        holdout_mape = safe_mape(y_holdout_original, holdout_pred)
                        holdout_mae = np.mean(np.abs(y_holdout_original - holdout_pred))
                        holdout_mse = np.mean((y_holdout_original - holdout_pred) ** 2)
                        holdout_rmse = np.sqrt(holdout_mse)

                        raw_int_final = final_model.intercept_ if hasattr(final_model, 'intercept_') else np.nan
                        raw_coefs_final = final_model.coef_.copy() if hasattr(final_model, 'coef_') else np.zeros(len(present_cols))

                        if isinstance(raw_coefs_final, pd.Series):
                            raw_coefs_final = raw_coefs_final.values

                        for i, col in enumerate(present_cols):
                            if col in final_scaler and i < len(raw_coefs_final):
                                mu, sd = final_scaler[col]
                                raw_coefs_final[i] = raw_coefs_final[i] / sd
                                raw_int_final -= raw_coefs_final[i] * mu

                        holdout_row = {k: v for k, v in zip(grouping_keys, gvals)}
                        holdout_row.update({
                            "Model": display_name,
                            "Fold": "Holdout",
                            "B0 (Original)": raw_int_final,
                            "R2 Holdout": holdout_r2,
                            "MAPE Holdout": holdout_mape,
                            "MAE Holdout": holdout_mae,
                            "MSE Holdout": holdout_mse,
                            "RMSE Holdout": holdout_rmse,
                        })

                        for i, col in enumerate(present_cols):
                            key_name = f"Beta_{col}"
                            if i < len(raw_coefs_final):
                                holdout_row[key_name] = raw_coefs_final[i]

                        rows.append(pd.DataFrame([holdout_row]))

                        holdout_preds = holdout_df.copy()
                        holdout_preds["Actual"] = y_holdout_original.values
                        holdout_preds["Predicted"] = holdout_pred
                        holdout_preds["Model"] = display_name
                        holdout_preds["Fold"] = "Holdout"
                        preds_records.append(holdout_preds)
                ...
                if isinstance(final_model, RecursiveLeastSquaresRegressor) and getattr(final_model, 'store_history', False):
                    if 'beta_history_store' not in st.session_state:
                        st.session_state.beta_history_store = {}
                    coef_history = [row.tolist() for row in getattr(final_model, 'coef_history_', [])]
                    intercept_history = [float(val) for val in getattr(final_model, 'intercept_history_', [])]
                    feature_names = list(getattr(final_model, 'feature_names_', []))

                    history_key = f"{group_display_name} | {display_name}"

                    if coef_history:
                        st.session_state.beta_history_store[history_key] = {
    ...
    for gvals, gdf in group_list_stacked:
        ...
        if use_holdout:
            if holdout_col_name and holdout_col_name in gdf.columns:
                ...
                if holdout_col_name and holdout_col_name not in gdf.columns:
                    st.warning(
                        f"ℹ️ {group_display_name}: Holdout column '{holdout_col_name}' not found. Using original order for stacked models."
                    )
                ordered_df = gdf.sort_index()

            if len(ordered_df) <= holdout_periods:
                st.warning(
                    f"⚠️ Skipping holdout for stacked model {group_display_name}: only {len(ordered_df)} samples,"
                    f" but holdout requires {holdout_periods}."
                )
                train_df = ordered_df.copy()
                holdout_df = pd.DataFrame()
        ...
        if stacked_models:
            ...
            n_groups = train_df[first_stacked_model.group_keys].drop_duplicates().shape[0]
        ...
        for mname, mdl in stacked_models.items():
            ...
            for fold_id, (tr_idx, te_idx) in enumerate(kf.split(X_full, y_full), 1):
                ...
                groups_tr = train_df.iloc[tr_idx][mdl.group_keys].copy()
                groups_te = train_df.iloc[te_idx][mdl.group_keys].copy()
                ...
                for group in unique_test_groups:
                    ...
                    if group in group_coefs:
                        ...
                        if group_mask_tr.any():
                            ...
                            group_train_data = train_df.loc[train_original_indices]
                        ...
                        pr = train_df.loc[test_original_indices].copy()
