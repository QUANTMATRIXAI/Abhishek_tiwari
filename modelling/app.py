...
def main():
    ...
    with col4:
        ...
        candidate_holdout_cols = []
        for col in df_working.columns:
            if col == target_col:
                continue
            col_dtype = df_working[col].dtype
            col_lower = col.lower() if isinstance(col, str) else str(col).lower()
            if (
                np.issubdtype(col_dtype, np.datetime64)
                or np.issubdtype(col_dtype, np.number)
                or any(keyword in col_lower for keyword in ['date', 'week', 'month', 'period', 'time'])
            ):
                candidate_holdout_cols.append(col)

        holdout_column_options = ['None'] + sorted(dict.fromkeys(candidate_holdout_cols))
        holdout_column_choice = st.selectbox(
            "Holdout Ordering Column:",
            options=holdout_column_options,
            index=0,
            help="Column used to order observations before reserving the holdout period."
        )

        holdout_periods = st.number_input(
            "Holdout Periods:",
            min_value=0,
            max_value=max(0, int(len(df_working) - 1)) if len(df_working) > 0 else 0,
            value=0,
            help="Number of trailing periods to reserve for out-of-sample testing."
        )

        if holdout_periods > 0 and holdout_column_choice == 'None':
            st.warning("⚠️ Holdout will use the current row order because no ordering column is selected.")

    selected_holdout_column = None if holdout_column_choice == 'None' else holdout_column_choice
    ...
    if st.button("▶️ RUN MODELS", type="primary", use_container_width=True):
        ...
        with st.spinner("Running models..."):
            ...
            results_df, predictions_df, lambda_df, ensemble_df = run_model_pipeline(
                ...
                holdout_column=selected_holdout_column,
    ...
    if st.session_state.results is not None:
        ...
        beta_history_store = st.session_state.get('beta_history_store', {})
        if beta_history_store:
            with st.expander("📉 RLS Coefficient History", expanded=False):
                st.caption("Visualize how Recursive Least Squares coefficients evolved across updates.")
                for combo_key, payload in beta_history_store.items():
                    st.markdown(f"**{combo_key}**")

                    feature_names = payload.get('feature_names', [])
                    coef_history = payload.get('coef_history', [])
                    intercept_history = payload.get('intercept_history', [])
                    train_obs = payload.get('train_observations')
                    holdout_obs = payload.get('holdout_observations')

                    if train_obs is not None or holdout_obs is not None:
                        st.caption(
                            f"Training observations: {train_obs or 0} | Holdout observations: {holdout_obs or 0}"
                        )

                    if coef_history:
                        history_df = pd.DataFrame(coef_history, columns=feature_names if feature_names else None)
                        history_df.insert(0, 'Step', np.arange(1, len(history_df) + 1))

                        if intercept_history and len(intercept_history) == len(history_df):
                            history_df.insert(1, 'Intercept', intercept_history)
                        elif intercept_history:
                            intercept_series = pd.Series(intercept_history).reindex(range(len(history_df))).fillna(method='ffill')
                            history_df.insert(1, 'Intercept', intercept_series.tolist())

                        st.dataframe(history_df, use_container_width=True, height=min(400, 100 + len(history_df) * 20))
                    else:
                        st.info("No coefficient history recorded for this run.")
