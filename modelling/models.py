...
class RecursiveLeastSquaresRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, forgetting_factor=1.0, initial_covariance=1e4,
        ...
        self.history_feature_names_ = None
    ...
    def fit(self, X, y):
        ...
        if self.store_history:
            ...
            self.history_feature_names_ = list(self.feature_names_) if self.feature_names_ is not None else None
    ...
    def _set_feature_metadata(self, X):
        ...
        self._non_positive_indices = [
            idx + offset for idx, name in enumerate(feature_names)
        ...
        self._non_positive_feature_names = {
            name for name in feature_names if name in configured_non_positive
        }
        self._non_negative_feature_names = {
            name for name in feature_names if name in configured_non_negative
        }
    ...
    def _apply_constraints(self):
        ...
        for idx in getattr(self, '_non_positive_indices', []):
            if idx < len(self._theta):
                self._theta[idx] = min(self._theta[idx], 0.0)

        for idx in getattr(self, '_non_negative_indices', []):
            if idx < len(self._theta):
                self._theta[idx] = max(self._theta[idx], 0.0)
